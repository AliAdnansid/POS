    ...
    private void bill_generateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bill_generateActionPerformed
        ...
        if(!cust_name.getText().isEmpty()&&
            ...
            for (int i = 0; i < prds.size(); i++) {
                ...
                String name = prds.get(i).getName();
                int amount = prds.get(i).getStock();
                if(productReport.get(name) != null){
                    productReport.put(name,amount);
                }
                else{
                    productReport.put(name,productReport.get(name) + amount);
                }
            }
            total_bill.setText(t_amount+"");   
            totalSale = totalSale + t_amount;
            totalTransaction++;
        }
        else
